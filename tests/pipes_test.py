...
class PipeTest(unittest.TestCase):
    ...
    def test_add_jobs(self):
        ...
        for (a, b) in zip(p.job_map.values(), expected):
            self.assertEqual(a, b, 'Jobset in the current stage of pipeline is different')
    ...
    def test_add_dependency(self):
        ...
        p2 = Pipe('dependency2')
        p.add_dependency(p2)
        self.assertEqual([p1, p2], p.dependent_on, 'Dependency not as expected')

        p3 = Pipe('dependency3')
        p4 = Pipe('dependency4')
        p.add_dependency(p3, p4)
        self.assertEqual([p1, p2, p3, p4], p.dependent_on, 'Dependency not as expected')
    ...
    def test_dependency_check(self):
        ...
        self.assertTrue(p.dependency(), 'Dependency check expected to be successful')

        p3.state = ThreadState.FAILED
        self.assertFalse(p.dependency(), 'Dependency check expected to be a failure')

    def test_representative_name(self):
        """Validates the name for pipeline."""
        p = Pipe('test')
        self.assertEqual('Pipe(test)', p.__repr__(), 'Pipe name not as expected')
        self.assertEqual('Pipe(test)', p.__str__(), 'Pipe name not as expected')
